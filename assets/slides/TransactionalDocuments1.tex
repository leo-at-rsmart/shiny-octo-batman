...
We have already created the skeleton for this data dictionary file for you.  Open the BookOrderDocument.xml file in the src/main/resources/train/bookstore/document/datadictionary directory.
Uncomment the body of this XML file so that both bean elements are uncommented.
\begin{enumerate} 
Notice how the TransactionalDocumentEntry associates the Document class that we created earlier with our workflow Document Type (which we will create in a later step).
Now open src/main/resources/trnapp-BookstoreModuleBeans.xml and add an entry to dataDictionaryPackages for your new data dictionary file.
Create JSP Page
Unlike maintenance documents, you have to create the JSP pages for transactional documents.  There are a set of standard tag libraries that can be used for this.  An initial BookOrder.jsp has been provided for you which includes the basic layout (without any custom fields yet).  We will add more fields a little bit later.
For now, let’s examine the JSP page:
Open the BookOrder.jsp page from the following location: src/main/webapp/WEB-INF/jsp
The <kul:documentPage> tag is the outermost tag for our documents.  It renders elements like the standard document header as well as the html form element for us.
The <kul:documentOverview> tag renders the top-most tab that shows up in all documents that allows the user to enter “description” and “explanation”.
<kul:notes> renders the “Notes and Attachments” tab on the document.
<kul:adHocRecipients> renders the tab that allows you to send adhoc workflow requests for the document
<kul:routeLog> renders that tab that contains an IFRAME showing the KEW Route Log for this document
<kul:documentControls> renders any relevant action buttons for the document (including save, approve, cancel, etc.)
Create a Link to the Book Order in the Portal
Open your bookstoreApplication.tag file located at src/main/webapp/WEB-INF/tags/rice-portal/channel/main
Add the following link to the top of the list:

<portal:portalLink 
  displayTitle="true"
  title="Book Order"
url="${ConfigProperties.application.url}/bookOrder.do?methodToCall=docHandler&command=initiate&docTypeName=BookOrderDocumentType" />
